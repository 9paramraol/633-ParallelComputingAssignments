...
void compute_edges(double *old, double *new, int N, int P, int my_row, int my_col, double *top, double *bottom, double *left, double *right){
    ...
    //finding for the top and the bottom row Note that the
    //computation for the corner points will be repeated but it does
    //not make a lot of difference in terms of performance
    for(int j = 0; j < N; j++){
        int i = 0;
        new[get_positn(i,j,N)] = find_new_for_point(i, j, N, old, left, right, top, bottom);
        i = N-1;
...
void print_in_file(FILE **fp, int myrow, int mycol, int N, int t, double* old){
    ...
    for(int r=0; r < N; ++r){
      for(int c=0; c < N; ++c)
        fprintf(*fp, "%d,%d,%d,%lf\n", t, (orow + r), (ocol + c), old[get_positn(r,c,N)]);
    }
    return;
}
