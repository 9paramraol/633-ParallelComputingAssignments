  ...
  start_time = MPI_Wtime ();
    ...
    for(int t = 0; t<Timesteps; ++t){
        ...
        if((mycol-1)>=0){
            position = 0;
            for(int i=0;i<N;++i)
                MPI_Pack((old[i] + 0), 1, MPI_DOUBLE, left_send_buff, N*double_size, &position, MPI_COMM_WORLD);
        ...
        if((mycol+1)<P){
            position = 0;
            for(int i=0;i<N;++i)
                MPI_Pack((old[i]+(N-1)), 1, MPI_DOUBLE, right_send_buff, N*double_size, &position, MPI_COMM_WORLD);
        ...
        if((myrow-1)>=0){
          position = 0;
          MPI_Pack(old[0], N, MPI_PACKED, top_send_buff, N*double_size, &position, MPI_COMM_WORLD);
        ...
        if((myrow+1)<P){
            position = 0;
            MPI_Pack(old[N-1], N, MPI_PACKED, bottom_send_buff, N*double_size, &position, MPI_COMM_WORLD);
